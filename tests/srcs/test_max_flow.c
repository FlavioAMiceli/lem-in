...
/*   Updated: 2020/07/04 19:04:59 by mmarcell      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
Test(max_flow, valid_normal_2_ants_5)
{
	...
	unsigned int	thresh_2 = 2;
	cr_assert_neq(path_2, NULL, "couldn't find vertix");
	t_vert			*path_3 = hmap_get(graph.vertices, "G");
	int				hops_3 = 4;
	...
	cr_assert_eq(graph.paths[2]->start, path_3, "second path saved: %s is not equal to second path expected: %s\n", graph.paths[2]->start->name, path_3->name);
	cr_assert_eq(graph.paths[2]->start->hops, hops_3, "first path hops saved: %d is not equal to first path hops expected: %d\n", graph.paths[2]->start->hops, hops_3);
	cr_assert_eq(graph.paths[2]->threshold, thresh_3, "threshold %d for first path doesn't match expected threshold %d\n", graph.paths[2]->threshold, thresh_3);
